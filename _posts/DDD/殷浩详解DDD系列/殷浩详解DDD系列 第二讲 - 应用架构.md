...
**简介：** 架构这个词源于英文里的“Architecture“，源头是土木工程里的“建筑”和“结构”，而架构里的”架“同时又包含了”架子“（scaffolding）的含义，意指能快速搭建起来的固定结构。而今天的应用架构，意指软件系统中**固定不变**的代码结构、设计模式、规范和组件间的通信方式。在应用开发中架构之所以是最重要的第一步，因为一个好的架构能让系统安全、稳定、快速迭代
...
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/cecd5f48cc8944ee60bd26b8e6a48bd9.png)
...
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/16392d1e285fd5b8f3b89cecb278f7cd.png)
...
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/c780b1ef579b78b89417a2d7820712bc.png)
...
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/d52a0c6ae0fdfa90beff14a235e23591.png)
// TODO：Graph

![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/e6abd5fd289c660aa82fdeb917c528bb.png)

#### 3.1.1 - Types模块

Types模块是保存可以对外暴露的Domain Primitives的地方。Domain Primitives因为是无状态的逻辑，可以对外暴露，所以经常被包含在对外的API接口中，需要单独成为模块。Types模块不依赖任何类库，纯POJO。
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/59a53da7e651770e2e1d96bafdf6a503.png)

#### 3.1.2 - Domain模块

Domain模块是核心业务逻辑的集中地，包含有状态的Entity、领域服务Domain Service、以及各种外部依赖的接口类（如Repository、ACL、中间件等。Domain模块仅依赖Types模块，也是纯POJO。
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/2fb3ec508cf09d36570c7378a69b9444.png)

#### 3.1.3 - Application模块

Application模块主要包含Application Service和一些相关的类。Application模块依赖Domain模块。还是不依赖任何框架，纯POJO。
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/a1c74996662b6850248744acc089dc36.png)
...
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/25e2a4843af643730270cbcf0feccf4b.png)

#### 3.1.5 - Web模块

Web模块包含Controller等相关代码。如果用SpringMVC则需要依赖Spring。
![image.png](殷浩详解DDD系列 第二讲 - 应用架构.assets/90cb0bdb71f971df80e8e805bea4e6c2.png)
